  ...
  return (
    <>
      <section className="mx-auto flex max-w-[1200px] flex-col gap-8 px-[clamp(20px,4vw,32px)] py-[clamp(56px,8vw,104px)]">
          ...
          <div role="group" aria-label={t.categoriesLabel} className="flex flex-wrap gap-2">
            {t.categories.map((c) => (
              <button
                ...
                aria-pressed={cat === c}
                onClick={() => setCat(c)}
                className={`min-h-11 cursor-pointer rounded-full border-[1.5px] px-4 text-[14px] font-bold transition-colors ${
                  cat === c ? 'border-ink bg-ink text-cream' : 'border-field-line bg-transparent text-ink hover:border-ink'
                }`}
              >
                {c}
              </button>
        ...
        {/* `key={cat}` : changer de catégorie remonte la grille, les cartes
            rejouent donc leur apparition. */}
        <div key={cat} className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,300px),1fr))] gap-6">
          {posts.map((p, i) => (
            <div key={`${p.tag}-${p.t}`}>
              ...
              {publishableGallery.map((photoItem, i) => (
                <Reveal key={photoItem.src} delay={(i % 3) * 90}>
                  <button
                    ...
                    onClick={() => setLb(i)}
                    className="ol-news-frame ol-gallery-frame block aspect-[4/5] w-full cursor-pointer overflow-hidden rounded-2xl border-0 p-0"
                  >
